...
int main() {
    ...
    //读取并预处理图片
    std::string image_path = R"(\path\to\image)";
    ...
    std::cout << "mask_feat_pred's size:" << mask_feat_pred.sizes() << std::endl;

    c10::List<c10::IValue> cate_preds = outputs[1].toList();
    std::cout << "cate_preds(List) contains" << cate_preds.size() << "elements" << std::endl;
    // 遍历列表，逐个元素处理
    ...
    for (size_t i = 0; i < cate_preds.size(); ++i) {
        ...
        std::cout << "cate" << i << "'s size:" << tensor_in_list.sizes() << std::endl;
        cate_list.push_back(tensor_in_list);
    }

    c10::List<c10::IValue> kernel_preds = outputs[2].toList();
    std::cout << "kernel_preds(List) contains" << kernel_preds.size() << "elements" << std::endl;
    // 遍历列表，逐个元素处理
    ...
    for (size_t i = 0; i < kernel_preds.size(); ++i) {
        ...
        at::Tensor tensor_in_list = element_ivalue.toTensor();
        std::cout << "kernel" << i << "'s size:" << tensor_in_list.sizes() << std::endl;
    ...
    try {
        ...
        if (seg_result.masks.defined()) {
            ...
            std::cout << "Labels: " << seg_result.labels.sizes() << std::endl;
            std::cout << "Scores: " << seg_result.scores.sizes() << std::endl;
            ...
            std::cout << "seg_pred dtype: " << seg_pred.dtype() << std::endl;
