...
from utils.utils import get_seg_mask

LineIdMap = {0: 0, 1: 1}

# Warning, do not use numpy random in PyTorch multiprocessing, or the random result will be the same.

# No need to use the dynamic category projection, for 
# [{'id': 0, 'name': '_background_', 'supercategory': None}, {'id': 1, 'name': 'Line', 'supercategory': None}]
...
class CocoIns(data.Dataset):
    def __init__(self, cfg):
        ...
        self.cate_ids = {v: k for k, v in LineIdMap.items()}
    ...
    def __getitem__(self, index):
        if self.mode == 'detect':
            ...
            for aa in target:
                ...
                category = LineIdMap[aa['category_id']]
