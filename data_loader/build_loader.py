...
class group_sampler:
    ...
    def _prepare_batches(self):
        ...
        # get a mask of the sampled elements
        mask = order >= 0

        # find the elements that belong to each individual cluster
        clusters = [(self.group_ids == i) & mask for i in self.groups]

        # get relative order of the elements in each cluster which follow the order from the sampler
        relative_order = [order[c] for c in clusters]
